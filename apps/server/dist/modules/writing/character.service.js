...
let CharacterService = class CharacterService {
    ...
    async deleteRelation(workId, relationId) {
        try {
            ...
            if (!relation) {
                throw new common_1.NotFoundException('关系不存在');
            }
    ...
    mapCharacterWithRelationsToResponse(character) {
        return {
            id: character.id,
            workId: character.workId,
            name: character.name,
            ...
            createdAt: character.createdAt,
            updatedAt: character.updatedAt,
            relationsFrom: (character.relationsFrom || []).map((r) => this.mapRelationToResponse(r)),
            relationsTo: (character.relationsTo || []).map((r) => this.mapRelationToResponse(r)),
        };
    }
    // 映射Relation为response格式
    mapRelationToResponse(relation) {
        return {
            id: relation.id,
